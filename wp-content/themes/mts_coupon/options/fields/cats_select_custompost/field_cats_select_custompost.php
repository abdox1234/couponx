<?php
class NHP_Options_cats_select_custompost extends NHP_Options{	
	...
	*/
	function render(){
		...
		if (isset($args['include_latest'])) {
			echo '<option value="latest"'.selected($this->value, 'latest', false).'>'.$args['include_latest'].'</option>';
		}
		
		$cats = get_terms($this->field['tax'], $args); 
		foreach ( $cats as $cat ) {
